...
def comparison(config: DatasetConfig, split: str) -> Comparison:
    """Every slice x metric of one dataset, both retrievers, from disk."""
    lex, sem = load(config, LEXICAL, split), load(config, SEMANTIC, split)
    agree(lex, sem)
...
def document(comparisons: list[Comparison], split: str) -> str:
    sections = [
        f"# Lexical versus semantic retrieval — {split} split",
        f"Generated by `python -m pipeline.compare --split {split}` from the "
        f"stored evaluation reports in "
        f"`artifacts/<dataset>/{evaluate.EVALUATE_DIR}/`. Nothing here was "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        comparisons = [comparison(config, args.split) for config in configs]
    except ComparisonError as error:
        print(f"\nerror: {error}\n", file=sys.stderr)
        return 2

    text = document(comparisons, args.split)
